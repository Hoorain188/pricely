...
import { api, ApiError, formatPrice, type ServerAlert } from './api/client';
...
export default function AlertsScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      // Signed in and scoped to this account on the server. This used to fall
      // back to a local copy, which could list alerts the server never had —
      // ones that looked set but that nothing would ever check.
  ...
  const handleRemove = async (alert: ServerAlert) => {
    setRemovingId(alert.id);
    setAlerts((prev) => prev.filter((a) => a.id !== alert.id));
    try {
      await api.deleteAlert(alert.id);
    } catch (err) {
      // Put it back: the server still has it, so it is still being watched.
      // Showing it gone would be the one outcome worse than a failed tap.
      setErrorMessage(err instanceof ApiError ? err.message : 'Could not remove that alert.');
  ...
  /**
   * Pause without deleting. Flipped immediately so the switch feels instant,
   * and flipped back if the server refuses — a switch that says off while the
   * alert still fires is the bug this replaced.
   */
  const handleToggle = async (alert: ServerAlert, next: boolean) => {
    setAlerts((prev) => prev.map((a) => (a.id === alert.id ? { ...a, isActive: next } : a)));
    try {
      await api.setAlertActive(alert.id, next);
    } catch (err) {
      setAlerts((prev) => prev.map((a) => (a.id === alert.id ? { ...a, isActive: !next } : a)));
      setErrorMessage(err instanceof ApiError ? err.message : 'Could not change that alert.');
      setErrorVisible(true);
    }
  };
      ...
      <>
        {alerts.map((item) => {
          ...
          return (
            <TouchableOpacity
              key={item.id}
              style={[styles.card, item.isTriggered && styles.cardHighlighted, !item.isActive && styles.cardInactive]}
              activeOpacity={0.9}
              onPress={() =>
            ...
            >
              <View style={styles.mainInfo}>
                ...
                <Switch
                  ...
                  trackColor={{ false: '#767577', true: colors.accentSolid }}
                  thumbColor="#FFFFFF"
                />
        ...
        {alerts
          .filter((a) => a.isTriggered && a.isActive)
          .map((alert) => (
